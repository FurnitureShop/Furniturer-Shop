...
export default function OrderReviewDraw({
  onClose,
  visible,
  data,
  onClickNavigatePaymentMethod,
}) {
  ...
  return (
    <Drawer
      className="order_preview_drawer"
      width={640}
      placement="right"
      onClose={onClose}
      visible={visible}
    >
      ...
      <div className="pt-9">
        <Steps
          current={checkStatusIndex(data.status)}
          labelPlacement={"vertical"}
        >
          <Steps.Step
            title="Create order"
            icon={<IconShoppingBag width="1em" height="1em" />}
          />
          <Steps.Step
            title={
              <>
                <p>Confirm payment method</p>
                {data.status === "Create order" ? (
                  <a
                    onClick={() => {
                      onClickNavigatePaymentMethod(data._id);
                    }}
                    style={{ fontSize: "0.8em", color: "rgba(0, 0, 0, 0.8)" }}
                  >
                    HERE
                  </a>
                ) : null}
            ...
            icon={<IconWallet width="1em" height="1em" />}
          />
          {data.status === "Cancelled" ? (
            <Steps.Step
              title="Cancelled"
              icon={<CloseCircleOutlined width="1em" height="1em" />}
            />
          ) : data.status === "Accepted" ? (
            <Steps.Step
              title="Accepted"
              icon={<CheckCircleOutlined width="1em" height="1em" />}
            />
          ) : (
            <Steps.Step
              title="Processing"
              icon={<IconProcessing width="1em" height="1em" />}
            />
          )}
          <Steps.Step
            title="Delivered"
            icon={<IconDeliveryTruck width="1em" height="1em" />}
          />
        </Steps>
      </div>
      <div className="text-3xl mb-3 mt-5">
